...
| leap_year_flg | INTEGER | 1 if leap year | 0 |
...
| full_dt | DATE | Actual date value | 2025-02-02 |
...
| TRADE_LEAP_WEEK_FLG | INTEGER | 1 if 53-week year | 0 |
...
INSERT INTO DIM_TRADE_DATE (
    DATE_KEY, full_dt, TRADE_YEAR_NUM, TRADE_WEEK_NUM,
...
SELECT
    ...
    MIN(full_dt) as week_start,
    MAX(full_dt) as week_end,
    DAYOFWEEK(MIN(full_dt)) as start_dow,
    DAYOFWEEK(MAX(full_dt)) as end_dow,
    'ERROR: Trade week is not Sunday-Saturday' as error_message
FROM DIM_TRADE_DATE
WHERE DATE_KEY > 0  -- Exclude special records
GROUP BY TRADE_WEEK_NUM, TRADE_YEAR_NUM
HAVING DAYOFWEEK(MIN(full_dt)) != 1  -- Not Sunday
    OR DAYOFWEEK(MAX(full_dt)) != 7;  -- Not Saturday
```
...
SELECT
    ...
    MAX(leap_year_flg) as leap_year_flg,
    CURRENT_TIMESTAMP as CREATE_TIMESTAMP
FROM DIM_DATE
WHERE DATE_KEY > 0
GROUP BY YEAR_NUM;
...
SELECT
    ...
    MAX(CASE WHEN TRADE_WEEK_OF_MONTH_445_NUM = 5 THEN 1 ELSE 0 END) as 5_WEEK_MONTH_445_FLG,
    MAX(CASE WHEN TRADE_WEEK_OF_MONTH_454_NUM = 5 THEN 1 ELSE 0 END) as 5_WEEK_MONTH_454_FLG,
    MAX(CASE WHEN TRADE_WEEK_OF_MONTH_544_NUM = 5 THEN 1 ELSE 0 END) as 5_WEEK_MONTH_544_FLG,
...
SELECT
    ...
    MAX(CONTAINS_LEAP_WEEK_FLG) as LEAP_WEEK_YEAR_FLG,
    CURRENT_TIMESTAMP as CREATE_TIMESTAMP

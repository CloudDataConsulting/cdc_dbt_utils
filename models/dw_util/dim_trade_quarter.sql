...
{{ config( post_hook="alter table {{ this }} add primary key (trade_quarter_key, calendar_pattern_txt)", ) }}

with trade_months as ( select * from {{ ref('dim_trade_month') }} where trade_month_key > 0 )  -- Exclude special records

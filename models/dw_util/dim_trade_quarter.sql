...
, quarter_445_aggregated as (
    select
        ...
        , min(
            case
                when trade_week_of_quarter_445_num > 4 and trade_week_of_quarter_445_num <= 8 then trade_month_445_num
            end
        ) as second_month_445_num
...
, quarter_454_aggregated as (
    select
        ...
        , min(
            case
                when trade_week_of_quarter_454_num > 4 and trade_week_of_quarter_454_num <= 9 then trade_month_454_num
            end
        ) as second_month_454_num
...
, quarter_544_aggregated as (
    select
        ...
        , min(
            case
                when trade_week_of_quarter_544_num > 5 and trade_week_of_quarter_544_num <= 9 then trade_month_544_num
            end
        ) as second_month_544_num
...
, final as (
    select
        ...
        , trade_pattern_txt
        || ' Pattern: Q'
        || trade_quarter_num::varchar
        || ' '
        || trade_year_num::varchar as trade_quarter_pattern_desc_txt
        ...
        , dense_rank()
            over (partition by trade_pattern_txt order by trade_quarter_start_dt)
            as trade_quarter_overall_num
